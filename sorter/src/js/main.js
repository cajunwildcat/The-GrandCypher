...
function init() {
	...
	document.addEventListener('keypress', (ev) => {
		...
		if (timestamp && !timeTaken && !loading && choices.length === battleNo - 1) {
			switch (ev.key) {
				...
				case 'k': case '1': pick('tie'); break;
				case 'j': case '2': undo(); break;
				default: break;
			}
		}
	...
	document.addEventListener('keydown', (ev) => {
		...
		if (timestamp && !timeTaken && !loading && choices.length === battleNo - 1) {
			switch (ev.key) {
				case 'ArrowLeft': ev.preventDefault(); pick('left'); break;
				case 'ArrowRight': ev.preventDefault(); pick('right'); break;
				case 'ArrowUp': ev.preventDefault(); pick('tie'); break;
...
async function setLatestDataset() {
	/** Set some defaults. */
	timestamp = 0;
	timeTaken = 0;
	choices = '';
	...
	characterData = await dataSet[currentVersion].characterData;
 ...
 */
async function decodeQuery(queryString = window.location.search.slice(1)) {
	let successfulLoad;
